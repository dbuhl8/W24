...
\item 
\begin{proof}
    In order to eliminate the third component of $a$ we look to find a vector who is equidistant from the $x,y$ plane and the vector $a$. That is we look for a vector $x$ between $a$ and the $xy$. Notice that there are an infinite number of choices to be made by taking the most obvious choice, $x = \frac{1}{\sqrt{17}}\left[\begin{array}{c} 2 \\ 3\\ 2\end{array}\right]$ and rotating it around the $z$-axis. Using this vector $x$ to build a Householder Reflector will project $a$ onto the x y plane, otherwise eliminating its third component. To prove this we look at $H$.
    \[
        H = \I - 2\frac{xx^T}{x^Tx} = \I - \frac{2}{17}xx^T
    \]
    \[
        Ha = a - \frac{2x^Ta}x = a - x
    \]
